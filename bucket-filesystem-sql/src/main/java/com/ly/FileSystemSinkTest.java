...
//		ds.print();
	...
	public void testFileJson() throws Exception {
		...
		tEnv.connect(
			new Bucket().basePath("/tmp/flink-data/json").rowFormat().dateFormat("yyyyMMddHHmm"))
...
//		tEnv.toAppendStream(table, Row.class).print();
	...
	public void testFileCsv() throws Exception {
		...
		char c = 0x01;
		tEnv.connect(new Bucket().basePath("file:///tmp/flink-data/csv").rowFormat()
...
//		TableSchema ts = TableSchema.fromTypeInfo(AvroSchemaConverter.convertToTypeInfo(Address.class));
		...
		tEnv.connect(new Bucket().basePath("/tmp/flink-data/avro").bultFormat()
			.dateFormat("yyyy-MM-dd-HHmm"))
//			.withFormat(new Avro().recordClass(Address.class))
			.withFormat(new Avro().avroSchema(
				"{\"type\":\"record\",\"name\":\"Address\",\"namespace\":\"org.apache.flink.formats.avro.generated\",\"fields\":[{\"name\":\"num\",\"type\":\"int\"},{\"name\":\"street\",\"type\":\"string\"}]}"))
			.inAppendMode()
			.registerTableSink("mysink");
...
//		tEnv.toAppendStream(result, Row.class).print();
		...
		tEnv.sqlUpdate("insert into mysink SELECT num,street FROM mysource");
//		tEnv.sqlQuery("SELECT city FROM mysource");
	 ...
	 */
	@Test
	public void testFileAvroNest() throws Exception {
		readNestingData();
...
//			.withFormat(new Avro().recordClass(Address.class))
			.withFormat(new Avro().avroSchema(
				"{\"namespace\":\"org.apache.flink.table.tests\",\"type\":\"record\",\"name\":\"UserInfo\",\"fields\":[{\"name\":\"user\",\"type\":\"string\"},{\"name\":\"event\",\"type\":{\"type\":\"record\",\"name\":\"event\",\"fields\":[{\"name\":\"type\",\"type\":\"string\"},{\"name\":\"message\",\"type\":\"string\"}]}}]}"))
			.inAppendMode()
			.registerTableSink("mysink");

		tEnv.sqlUpdate("insert into mysink SELECT user,event FROM mysource");
	...
	public void testFileParquet() throws Exception {
		...
		tEnv.connect(new Bucket().basePath("hdfs:///tmp/flink-data/parquet").bultFormat()
			.dateFormat("yyyy-MM-dd-HHmm"))
...
//			.registerTableSource("mysource");
	...
	private void readNestingData() {

		DataStream<Tuple3<String, String, Row>> dataStream = env
			.addSource(new MySourceNestingAvro()).returns(org.apache.flink.api.common.typeinfo.Types
				.TUPLE(
					org.apache.flink.api.common.typeinfo.Types.STRING,
					org.apache.flink.api.common.typeinfo.Types.STRING,
					org.apache.flink.api.common.typeinfo.Types
						.ROW(org.apache.flink.api.common.typeinfo.Types.STRING,
							org.apache.flink.api.common.typeinfo.Types.STRING)));
		tEnv.registerDataStream("mysource", dataStream, "rowtime,user,event");
	 ...
	 *
	 */
	public static class MySourceNestingAvro implements SourceFunction<Tuple3<String, String, Row>> {
		...
		@Override
		public void run(SourceContext<Tuple3<String, String, Row>> ctx)
			throws Exception {
			while (isRunning) {
				Thread.sleep((int) (Math.random() * 100));
				...
				String user = "user" + n++;
				Row row = new Row(2);
				row.setField(0, "WARNING");
				row.setField(1, "message");
				Tuple3<String, String, Row> tuple5 = Tuple3.of(date1, user, row);
				ctx.collect(tuple5);
			}
		}
	...
	@Test
	public void test111(){
		String schemaString = ReflectData.get().getSchema(UserInfo.class).toString();
