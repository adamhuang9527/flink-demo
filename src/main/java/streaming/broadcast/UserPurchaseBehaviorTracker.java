...
public class UserPurchaseBehaviorTracker {
	...
	private static final Logger LOG = LoggerFactory.getLogger(UserPurchaseBehaviorTracker.class);
	private static final MapStateDescriptor<String, Config> configStateDescriptor = new MapStateDescriptor<>(
			"configBroadcastState", BasicTypeInfo.STRING_TYPE_INFO, TypeInformation.of(new TypeHint<Config>() {
			}));
...
//        "--group.id", "customer-purchase-behavior-tracker" };
		...
		env.setStateBackend(
				new FsStateBackend("hdfs://namenode01.td.com/flink-checkpoints/customer-purchase-behavior-tracker"));
		...
		final FlinkKafkaConsumer010 kafkaUserEventSource = new FlinkKafkaConsumer010<>(
				parameterTool.getRequired("input-event-topic"), new SimpleStringSchema(),
		...
		final FlinkKafkaConsumer010 kafkaConfigEventSource = new FlinkKafkaConsumer010<>(
				parameterTool.getRequired("input-config-topic"), new SimpleStringSchema(),
				parameterTool.getProperties());
		...
		final FlinkKafkaProducer010 kafkaProducer = new FlinkKafkaProducer010<>(
				parameterTool.getRequired("output-topic"), new EvaluatedResultSchema(), parameterTool.getProperties());
	...
	private static class ConnectedBroadcastProcessFuntion
		...
		private final MapStateDescriptor<String, Map<String, UserEventContainer>> userMapStateDesc = new MapStateDescriptor<>(
				"userEventContainerState", BasicTypeInfo.STRING_TYPE_INFO,
		...
		@Override
		public void processElement(UserEvent value, ReadOnlyContext ctx, Collector<EvaluatedResult> out)
				throws Exception {
			...
			final MapState<String, Map<String, UserEventContainer>> state = getRuntimeContext()
					.getMapState(userMapStateDesc);
			...
			// if true, then compute the purchase path length, and prepare to trigger
			// predefined actions
